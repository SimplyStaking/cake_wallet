          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Observer(builder: (_) {
                final onlyDecentralized = decentralizedOnly();
                return Column(
                  spacing: 24,
                  children: [
                    ...
                    NewListSections(sections: {
                      '': [
                        for (final provider in PegarouteProviderPreferences.providers.entries)
                          if (provider.key == 'instaswap' && onlyDecentralized)
                            ListItemRegularRow(
                              keyValue: provider.key,
                              label: provider.value,
                              subtitle: S.of(context).decentralized_only,
                              showArrow: false,
                            )
                          else
                            ListItemToggle(
                              ...
                              leadingEndWidget: Text(
                                provider.key == 'instaswap'
                                    ? 'ETH · ${S.of(context).centralized}'
                                    : S.of(context).coming_soon_tag,
